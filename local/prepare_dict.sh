...
stage=0
nj=40 # number of parallel Sequitur G2P jobs, we would like to use
cmd=run.pl
...
cmuDict=$lm_dir/cmudict-0.7b
vocab=data/data_all/word.list
lexicon_raw_nosil=$dst_dir/lexicon_raw_nosil.txt
libspeech_lex=/data/002.data.beiping.shu/kaldi/egs/librispeech/s5/data/local/lm/librispeech-lexicon.txt
...
if [ $stage -le 4 ];then
  ...
  awk 'NR==FNR{a[$1] = 1; next} !($1 in a)' $libspeech_lex $vocab |\
  ...
  #bash local/g2p.sh $g2p_dir/vocab_autogen.full $g2p_model_dir $g2p_dir/lex.tmp
  g2p_vocab_size=$(wc -l <$g2p_dir/vocab_autogen.full)
  g2p_lex_size=$(wc -l < <(cat $auto_lexicon_prefix.*))
  [[ "$g2p_vocab_size" -eq "$g2p_lex_size" ]] || { echo "Unexpected G2P error"; echo "unkown-vocab's size is " $g2p_vocab_size " but G2P can get " $g2p_lex_size;}
  sort <(cat $auto_vocab_prefix.*) >$dst_dir/vocab_autogen.txt
  sort <(cat $auto_lexicon_prefix.*) >$dst_dir/lexicon_autogen.txt
  #sort <(cat $g2p_dir/lex.tmp) >$dst_dir/lexicon_autogen.txt
  echo "$(wc -l <$g2p_dir/vocab_autogen.full) pronunciations autogenerated OK"
fi
  
if [ $stage -le 5 ]; then
  echo "Combining the libspeech-lexicon pronunciations with the autogenerated ones ..."
  awk 'NR==FNR{a[$1]=1; next} ($1 in a)' $vocab $libspeech_lex |\
...
if [[ ! -s "$lexicon_raw_nosil" ]]; then
    cp $libspeech_lex $lexicon_raw_nosil || exit 1
fi
